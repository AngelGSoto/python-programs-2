...
#Labels
def label(label, posx, posy, ax):
    for label_, x, y in zip(label, posx, posy):
         ax.annotate(label_, (x, y), alpha=0.9, size=5,
                     xytext=(-1.0, -8.0), textcoords='offset points', ha='right', va='bottom', verticalalignment='center', zorder=300,)
...
#tab_ = hdu[1].data
tab_ = Table.read(file_, format="ascii")
...
ax.scatter(colours0["r-i"], colours0["r-ha"], c=sns.xkcd_rgb['cerulean'], edgecolor='white', alpha=0.9, marker ='o', s=50, zorder=211.0, label='SMC PNe')
...
label(tab_['Seq'], colours0["r-i"], colours0["r-ha"], ax)
...
plt.tight_layout()
pltfile = 'figs-PN-pca/Fig1-{}-vironen.pdf'.format(file_.split('.da')[0])
...
ax1.scatter(colours0["F515 - F861"], colours0["F515 - F660"], c=sns.xkcd_rgb['cerulean'], edgecolor='white', alpha=0.9, marker ='o', s=50,  zorder=211.0, label='PN candidate')
...
label(tab_['Seq'], colours0["F515 - F861"], colours0["F515 - F660"], ax1)
...
#ax1.grid()
plt.tight_layout()
pltfile = 'figs-PN-pca/Fig2-{}-J0515_J0660.pdf'.format(file_.split('.da')[0])
...
ax2.scatter(colours0["z - g"], colours0["z - F660"], c=sns.xkcd_rgb['cerulean'], edgecolor='white', alpha=0.9, marker ='o', s=50,  zorder=211.0, label='PN candidate')
...
label(tab_['Seq'], colours0["z - g"], colours0["z - F660"], ax2)
...
plt.tight_layout()
plt.tight_layout()
pltfile = 'figs-PN-pca/Fig3-{}-z.pdf'.format(file_.split('.da')[0])
#file_save = os.path.join(save_path, pltfile)
plt.savefig(pltfile)
plt.clf()
###############################################################
...
ax4.scatter(colours0["F660 - r"], colours0["g - F515"], c=sns.xkcd_rgb['cerulean'], edgecolor='white', alpha=0.9, marker ='o', s=50,  zorder=211.0, label='PN candidate')
...
label(tab_['Seq'], colours0["F660 - r"], colours0["g - F515"], ax4)
...
#sns.despine(bottom=True)
plt.tight_layout()
pltfile = 'figs-PN-pca/Fig4-{}-g.pdf'.format(file_.split('.dat')[0])
#file_save = os.path.join(save_path, pltfile)
plt.savefig(pltfile)
plt.clf()
...
ax5.scatter(colours0["g - i"], colours0["F410 - F660"], c=sns.xkcd_rgb['cerulean'], edgecolor='white', alpha=0.9, s=50,  zorder=211.0, label='PN candidate')
...
label(tab_['Seq'], colours0["g - i"], colours0["F410 - F660"], ax5)
...
plt.tight_layout()
plt.tight_layout()
pltfile = 'figs-PN-pca/Fig5-{}-gi.pdf'.format(file_.split('.da')[0])
#file_save = os.path.join(save_path, pltfile)
plt.savefig(pltfile)
